...
iput(struct inode *ip)
{
  ...
  if (ip->ref == 1 && ip->valid && ip->nlink == 0) {
    /*递减ref后没有指针指向该inode，并且没有任何目录项链接该inode，则该inode和它的数据块必须被释放*/
    /*递减后的引用数为0，inode缓存将释放掉该inode在inode缓存中的槽位，该槽位就可以被其他inode使用。*/
    // inode has no links and no other references: truncate and free.

    // ip->ref == 1 means no other process can have ip locked,
    // so this acquiresleep() won't block (or deadlock).
    acquiresleep(&ip->lock);/*锁定该inode*/

    release(&itable.lock);

    /*更新superlock中的列表*/
    if (sb.lzrni_list_num > NINODE) {/*故障检测*/
    ...
    if (i == sb.lzrni_list_num && !flag) {/*故障检测*/
      panic("can't find expected inode in lzrni_list");
    }

    //print_superblock_list();/*显示效果*/

    itrunc(ip);/*将文件截断为0字节，释放数据块*/
    ip->type = 0;/*设置为未分配*/
    iupdate(ip);/*将该inode写入磁盘*/
    ip->valid = 0;/*设置为未从磁盘上拷贝相应信息*/

    releasesleep(&ip->lock);/*释放这个inode的睡眠锁*/
...
clean_lzrni_list() {
  ...
  sb.lzrni_list_num = 0;/*清空superblock中的列表*/
}

int/*居然没有bool类型*/
inum_exist(uint inum_temp) {/*检查待加入的inode是否已经在superblock的列表中*/
  for (int i = 0; i < sb.lzrni_list_num; i++) {
...
void
lzrni_list_expand(uint inum_temp) {/*向superblock列表中加入符合条件的inum*/
